...
class iContactApi {
	...
	**/
	 ...
	 * @return object
	 **/
	public function subscribeContactToList($iContactId, $iListId, $sStatus = 'normal', $confirmationMessageId) {
		...
		if (!empty($confirmationMessageId))
			$msg['confirmationMessageId'] = $confirmationMessageId;
	...
	**/
	...
	/**
	 * Check whether contactId exists in a list
	 * @param unknown_type $iContactId
	 * @param unknown_type $iListId
	 */
	public function getSubscription($iContactId, $iListId) {
		// Make the call and return the data
		return $this->makeCall("/a/{$this->setAccountId()}/c/{$this->setClientFolderId()}/subscriptions??listId={$iListId}&contactId={$iContactId}", 'GET');
	}
